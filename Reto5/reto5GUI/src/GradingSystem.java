...
public class GradingSystem{
    ...
    public HashMap<String, List<Double>> stat2(List<List<Double>> universe) {
    ...
    public List<List<Double>> castData(List<Student> estudiantes){
        List<List<Double>> answer = new LinkedList<List<Double>>();
        for (var i = 0; i < estudiantes.size(); i++) {
            Student data_student = estudiantes.get(i);
            for (var j = 0; j < data_student.getAsignaturas().size(); j++) {
                List<Double> double_cast = new LinkedList<Double>();
                double_cast.add(Double.parseDouble(data_student.getNombre()));
                double_cast.add(Double.parseDouble(data_student.getGenero()));
                double_cast.add(Double.parseDouble(data_student.getAsignaturas().get(j)));
                double_cast.add(Double.parseDouble(data_student.getNotas().get(j)));
                answer.add(double_cast);
            }
        }
